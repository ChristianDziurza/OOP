...
public class Exercicio1 extends Application {
    ...
    public void start(Stage stage) {
        ...
        Label lblMsg = new Label("Aguardando informações");
        grid.setHgap(7);
        grid.setVgap(7);
        ...
        ObservableList<String> cursos = FXCollections.observableArrayList("Ciência da Computação", "Matemática",
                "Matemática Aplicada", "Engenharia de Alimentos");
        chcCursos.setItems(cursos);
        chcCursos.getSelectionModel().select(0);
        grid.add(chcCursos, 1, 3);
        root.setCenter(grid);
        btnGravar.setOnAction(e->{
            lblMsg.setText("Informações salvas para "+ txtNome.getText());
        });
